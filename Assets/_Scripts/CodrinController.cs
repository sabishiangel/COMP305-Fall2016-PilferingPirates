...
public class CodrinController : MonoBehaviour
{
    ...
    private void _Move()
    {
        this.transform.position = new Vector2(Mathf.Clamp(transform.position.x, -280, 280), Mathf.Clamp(transform.position.y, -190, 195)); //Clamps movement to one area         

        transform.position = Vector2.MoveTowards(transform.position, player.position, _speed * Time.deltaTime+20);
